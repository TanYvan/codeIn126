...
class RemindController < ApplicationController
  ...
  def will_list
    ...
    @remind_pages,@remind=paginate(:reminds,:conditions=>"used='Y' and (state = 1 and state = 2) and uu='#{session[:user_code]}'  and dt1>'#{Date.today.to_s(:db)}'",:order=>@order,:per_page=>@page_lines.to_i)
  end
  #历史提醒
  def histroy_list
    @order=params[:order]
