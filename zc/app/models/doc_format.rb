...
#        end
  ...
  def format_letter_fmt02_1(code)
    ...
    anhao=@case.case_code
    iNumber=anhao[-7,7].to_i
    
    if iNumber >= 2014205 and iNumber <= 2014761
    	@format_fmt02_1 = add_record("<12$$$>","惠州市惠阳区淡水“中港城”系列案件材料的原件在H2014760案件中。",@format_fmt02_1)
    else
    	@format_fmt02_1 = add_record("<12$$$>","",@format_fmt02_1)
    end 
...
#        @arbitman_fee_rmb = TbShouldCharge.sum("rmb_money - redu_rmb_money",:conditions => "recevice_code='#{code}' and typ='0008' and used='Y' " )    
  ...
  def format_letter_dfor03(code)
    ...
    @strJigou = get_Jigou(code)
    @format_dfor03 = add_record("<29$$$>",@strJigou,@format_dfor03)
    @re = get_remark(code)
...
#      end
  ...
  def format_letter_for03(code,doc_id)
    ...
    @strJigou = get_Jigou(code)
    @format_for03 = add_record("<29$$$>",@strJigou,@format_for03)    
    @re = get_remark(code)
...
#    @format_dfmt92 = add_record("<02$$$>",@comment_date,@format_dfmt92)
  ...
  def format_letter_fmt57(code)
    aaa="3. 本会已为您在深圳皇庭V酒店预订雅致大床房（地址和联系方式见备注），您也可以参照每晚不超过人民币900元的标准自行在深圳市安排住宿，本会将根据您提供的票据据实报销。"
    @format_fmt57 = ""
    @case = TbCase.find_by_recevice_code(code)
  ...
  def format_letter_fmt58(code)
    aaa="3. 本会已为您在深圳皇庭V酒店预订雅致大床房（地址和联系方式见开庭通知的备注），您也可以参照每晚不超过人民币900元的标准自行在深圳市安排住宿，本会将根据您提供的票据据实报销。"
    @format_fmt58 = ""
    @case = TbCase.find_by_recevice_code(code)
    unless @case
      return ""
...
#    @arb.each{|a| arbit = arbit + " " + TbArbitman.find_by_code(a.arbitman).name_idcard}
  ...
  def format_letter_fmt70(code,doc_id)
    ...
    #申请人  格式：第一申请人：张XX
    @applicants,@applicants2,numbers = applicant_awards(code,2,3)    
    @applicants = @applicants.gsub("(","（")
 		@applicants = @applicants.gsub(")","）")              		    
    ...
    #申请人  格式：张XX    
    if numbers>1 then
    	@applicants2=@applicants2+"（上述申请人以下合称为“申请人”）"    	
    else
    	if @applicants2[-1,1]==")"
      	@applicants2=@applicants2.insert(-2,"，下称“申请人”") 
      else	
      	@applicants2=@applicants2+"（下称“申请人”）"
    	end 
    end 
    #括号格式调整    
  	@applicants2 = @applicants2.gsub("(","（")
		@applicants2 = @applicants2.gsub(")","）")                    
    @format_fmt70 = add_record("<05$$$>",@applicants2,@format_fmt70)        
    #仲裁费
    @arbit_fee = applicant_arbit_fee22(code,'0001')
    @arbit_fee=SysArg.compart1(@arbit_fee)
    ...
    #独任仲裁员    
    c1 = independent_arbitrator(code) + get_arbitman_sex(code,'0001')
	  @format_fmt70 = add_record("<08$$$>",c1,@format_fmt70)        
		#合同签订日期
		#get_contract_date
		iLocationofDate=@contract_name.index('日')
		sSignDate=@contract_name[0,iLocationofDate+3]
		@format_fmt70 = add_record("<09$$$>",sSignDate,@format_fmt70)   
		#合同编号及名称
		sContractName=@contract_name[iLocationofDate+12,@contract_name.length]
		@format_fmt70 = add_record("<10$$$>",sContractName,@format_fmt70)   
		#助理
    #@zl_alerk = get_name(code)+get_zlsex(code)
    #@telephone = get_tel(code,1)
    #@fax = get_fax(code,1)
  ...
  #申请人   v1-1:以分号隔开,在函中；2：分别标明第几个当事人，以换行符结尾。3:分别标明第几个当事人，以顿号结尾，用于函中。v3:3:要冒号 4：不要冒号
      ...
      @applicants.each do |applicant|
        if @applicants.length>1
          if v1==1
            ...
            if i==@last1
              @applicant1=@applicant1+"第"+SysArg.n_to_md(i)+"申请人"+ applicant.partyname + "  "
            else
              @applicant1=@applicant1+"第"+SysArg.n_to_md(i)+"申请人"+ applicant.partyname + "、"
            end
          end
  ...
  #申请人   v1-1:以分号隔开,在函中；2：分别标明第几个当事人，以换行符结尾。分别标明第几个当事人，以顿号结尾，用于函中。v3:3:要冒号 4：不要冒号
  def applicant_awards(code,v1,v3)
    @applicants = TbParty.find(:all,:conditions => ["used = 'Y' and partytype = 1 and recevice_code = ?",code], :order => "id")
    if @applicants == nil
      @applicant1 = "_____"
      @applicant2 = "_____"
      ...
      @applicants.each do |applicant|
        if @applicants.length>1
          if v1==1
            ...
            @applicant1 =@applicant1+"第"+SysArg.n_to_md(i)+"申请人："+ applicant.partyname + '\r'                      
            if i==@last1
              @applicant2=@applicant2+"第"+SysArg.n_to_md(i)+"申请人"+ applicant.partyname + "  "
            else
              @applicant2=@applicant2+"第"+SysArg.n_to_md(i)+"申请人"+ applicant.partyname + "、"
            end
          end
          ...
          if v3==3
            @applicant1="申请人："+applicant.partyname + "  "
            @applicant2=applicant.partyname + "  "
          elsif v3==4
            @applicant1=applicant.partyname + "  "
          end
        end
      ...
      if @applicant1!=""
        ...
        @applicant2="申请人：__________"
      end      
    end

    return @applicant1,@applicant2,@applicants.length
  end
  ...
  def get_accommodate_remarks(code,v1)
    ...
    if PubTool.new.get_first_record(@party)!=nil
      ...
      if @isperson!=nil#个人
        if @len1==1
          ...
          if @aa!=nil and @bb!=nil
            str1="营业执照和组织机构代码证/公司注册和商业登记证明"
          elsif @aa!=nil and @bb==nil #国内的
            str1="身份证明/营业执照和组织机构代码证"
          else#港澳与国外的
            str1="公司注册和商业登记证明"
          end
        end
      ...
      str1="身份证明/营业执照和组织机构代码证/公司注册和商业登记证明"
    end
    return str1
  end
...
# 开庭地点
  ...
  #20150723 回传选择的仲裁机构名称函数
  def get_Jigou(code)
    @case=TbCase.find(:first,:conditions => ["used = 'Y' and recevice_code = ?",code])
    t_jigou = @case.agent
    if t_jigou!=nil and t_jigou!=""      
      @jigou = TbDictionary.find(:first, :conditions => ["state='Y' and typ_code='0004' and data_val=?",t_jigou]).data_text
		end    
    return @jigou  
	end
  #咨询助理的相关信息###################################################
 ...
 s.used='Y' and s.typed='#{typ}' and s.recevice_code='#{code}' and s.request_typ='#{p_v}' order by s.id")
  ...
  def get_send_code(code)
    ...
    if endbook
      ...
      sYear=s[3,4]
      get_amount = (sYear.to_i >= 2015 ? 4 : 3)
      iNumber=s[7,get_amount].to_i
      sResult = "[" + sYear + "]"
