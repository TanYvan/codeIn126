...
class CaseDocController < ApplicationController
  ...
  def new
    ...
    if @case_code==nil or @case_code==""
      @typ=TbDocFormat.find_by_sql("select code,name,spell,public_to from tb_doc_formats where used='Y' and (aribitprog_code like '%0000,%' or aribitprog_code like '%" + @case.aribitprog_num + ",%') and (role_code like '%0000,%' or role_code like '%" + @case.app_rules + ",%') and (sub_code='ft04' or sub_code='fr03' or sub_code='fmt04' or sub_code='for03' or sub_code='dfmt04' or sub_code='dfor03' or sub_code='fmt20' or sub_code='dfmt51' or sub_code='fmt53') and sub_code<>'' order by ind,code")
    else
      ...
      @typ=TbDocFormat.find(:all,:conditions=>condi_condi,:select=>"code,name,spell,public_to",:order=>"ind,code")
    end
    #合同编号、名称
    ...
    @ccc=""
    @typ.each{|t|
      @ccc = @ccc + "if(code==\"#{t.code}\"){ a=\"#{t.public_to}\"};\n"
    }
  end

  def create
    @doc=TbDoc.new(params[:doc])
    @doc.public_to=params[:public_to]
  ...
  def opera
    ...
    @case_code = TbCase.find_by_recevice_code(params[:recevice_code]).case_code
    if @case_code==nil or @case_code==""
      @typ=TbDocFormat.find_by_sql("select code,name,public_to from tb_doc_formats where used='Y' and  (aribitprog_code like '%0000,%' or aribitprog_code like '%" + @case.aribitprog_num + ",%') and (role_code like '%0000,%' or role_code like '%" + @case.app_rules + ",%') and  (sub_code='ft04' or sub_code='fr03' or sub_code='fmt04' or sub_code='for03' or sub_code='dfmt04' or sub_code='dfor03' or sub_code='fmt20' or sub_code='dfmt51' or sub_code='fmt53') and sub_code<>'' order by ind,code")
    else
      if ["0001","0002","0003","0004"].index(@case.aribitprog_num )
        if TbCasearbitman.count("used",:conditions => "used='Y' and recevice_code='" + @case.recevice_code + "'" ).to_i > 1
          condi_condi = "used='Y' and ((aribitprog_code like '%0000,%' or aribitprog_code like '%" + @case.aribitprog_num + ",%') and (role_code like '%0000,%' or role_code like '%" + @case.app_rules + ",%') and code<>'0090' ) and sub_code<>''"
