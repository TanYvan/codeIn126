class CaseQuery1Controller < ApplicationController   
  ...
  def case_now_clerk2_total
    ...
    if @search_condit!= " and " and p.sql_check_3(@order)!=false
      ...
      @case2=TbCase.find_by_sql("select users.name as clerk_name, (select count(1) from tb_cases  where tb_cases.clerk=users.code  and  tb_cases.used='Y' and tb_cases.state>=4 and tb_cases.state<100 and tb_cases.end_code='' and tb_cases.aribitprog_num in ('0001','0003','0005','0007') and  tb_cases.clerk=users.code) as n1 ,  (select count(1) from tb_cases  where tb_cases.clerk=users.code  and  tb_cases.used='Y' and tb_cases.state>=4 and tb_cases.state<100 and tb_cases.end_code='' and tb_cases.aribitprog_num in ('0002','0004','0006','0008') and  tb_cases.clerk=users.code) as n2 from users where users.code in (select distinct tb_cases.clerk from tb_cases,users where tb_cases.clerk=users.code and  tb_cases.used='Y' and tb_cases.state>=4 and tb_cases.state<100 and tb_cases.end_code='')")
  ...
  def list_31
    ...
    @order=params[:order]
    if @order==nil or @order==""
      #@order="right(tb_cases_case_code,8) desc" 
  ...
  def list_32_a
    ...
    if params[:name].blank?
      ...
      c="(party1_partyname like '%#{params[:name]}%' or party2_partyname like '%#{params[:name]}%') and( tb_cases_state>=4 or tb_cases_state=1) and tb_cases_recevice_code<>'#{params[:recevice_code]}'"
    end
    
    @case=VCaseQuery1List2.find_by_sql("select distinct tb_cases_state,tb_cases_recevice_code,tb_cases_general_code,tb_cases_case_code, tb_cases_end_code,tb_cases_clerk from v_case_query1_list2s where #{c}  order by tb_cases_case_code asc")
