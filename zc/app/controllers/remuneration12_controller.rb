class Remuneration12Controller < ApplicationController
  ...
  def create_1_a
    extend = TbBonusPenalty.find(:first,
      :conditions => "recevice_code='#{session[:recevice_code_1]}' and typ='0002'
                      and p='#{params[:remun][:p_code]}' and used='Y' and extend_code<>''"
    ) # 当前案件session[:recevice_code_1] 当前仲裁员params[:remun][:p_code] 且 已经发放

    # 不可以录入相同的记录
    rem1 = RemunerationAward.find(:all,:conditions => ["recevice_code=? and p_typ='0002' and p_code=? and typ=? and used='Y'",session[:recevice_code_1],params[:remun][:p_code],params[:remun][:typ]])
    rem2 = RemunerationAward.find(:all,:conditions => ["recevice_code=? and p_typ='0002' and typ=? and used='Y'",session[:recevice_code_1],params[:remun][:typ]])
    ...
    if rem1.size != 0
      ...
      if extend == nil # 该仲裁员报酬未发放  如果报酬已经发放则不可以创建
        ...
        @remun.t1            = Time.now()
        if @remun.save
          TbBonusPenalty.add(session[:recevice_code_1], '0002', params[:remun][:p_code])
          flash[:notice] = "创建成功"
          redirect_to :action => "list_1"
        else
          flash[:notice] = "创建失败"
  ...
  def create_1_all
    ...
    if awarddetail.blank? or awarddetail.size == 0
      ...
      #############
      unless award.blank?
        awarddetail = TbAwardDetail.find(:first,
          :order => 'typ,draftsman_typ,draftsman',
          :conditions => ["recevice_code=? and used='Y' and p_id=? and draftsman_typ='0002' ", session[:recevice_code_1], award.id]
        )
        if awarddetail!=nil
          remun  = RemunerationAward.new
          remun.used          =  'Y'
          ...
          remun.typ           = "0001" if awarddetail.typ=="0003"
          remun.typ           = "0002" if awarddetail.typ=="0002"
          remun.typ           = "0004" if awarddetail.typ=="0001"
          remun.typ           = "0003" if awarddetail.typ=="0005"
          remun.typ           = "0005" if awarddetail.typ=="0004"
          
          remun.p_code        = awarddetail.draftsman
          #remun.arbitman_type = arbitman[ad.draftsman]
          remun.remark        = ""
          if remun.typ == "0001"
            remun.grade_xs = 100
          ...
          remun.t1            = Time.now()
          if remun.save()
            TbBonusPenalty.add(session[:recevice_code_1], '0002',awarddetail.draftsman)
          end
        end
      end
      #############
