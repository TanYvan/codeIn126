...
#创建时间 20090522
    ...
    def list_arbitman           
        ...
        @hant_search_1=[['char','tb_arbitmen_name','姓名','text',[]],
          ...
          ['char','tb_arbitmen_area_code','地区','selecttext',Region.find(:all).collect{|p|[p.code,p.name]}.insert(0,["",""])],
#          ['char','name','年龄','text',[]],    
...
#          ['char','name','一年内实际承接案件数','text',[]]
    ...
    def show_photo
        ...
        #显示仲裁员精通专业信息
        def list_industry
          @arbitman_num = params[:id]
          @tb_arbitman = TbArbitman.find(@arbitman_num)
          @industry = Industry.find(:all, :conditions =>["arbitman_num = ? and used = 'Y'",@tb_arbitman.id],:order => "id")
        end
        #仲裁员简历的列表
        def list_resume
            @tb_arbitman = TbArbitman.find(params[:id])
