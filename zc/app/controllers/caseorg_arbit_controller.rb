...
class CaseorgArbitController < ApplicationController
  ...
  def arbitman_new
    ...
    @arbitman_select = CaseArbitmanSelect.find(:all, :conditions=>["recevice_code=? ", session[:recevice_code]], :order => "id")
    @user_type = {1=>"申请方",2=>"被申请方"}
    @used = {"Y"=>"","N"=>"已删除"}
  end

  def select_delete
    @select = CaseArbitmanSelect.find(params[:id])
    if @select.recevice_code==session[:recevice_code]
       @select.used="N"
       @select.u=session[:user_code]
       @select.u_t=Time.now()
       if @select.save
         flash[:notice]="逻辑删除成功"
       else
    ...
    redirect_to :action=>"arbitman_new"
  end
