class Remuneration12SetController < ApplicationController
    ...
    if session[:recevice_code_2]!=nil
      ...
      if ca_rc == nil or @xs.blank?
        ...
        if (aribitprog=='0003' || aribitprog=='0004' || aribitprog=='0007'|| aribitprog=='0008') && ! TbShouldCharge.find_by_sql("select tb_should_charges.id from tb_should_charges,tb_charges where tb_should_charges.recevice_code='#{ca_rc}' and tb_should_charges.id=tb_charges.should_id and tb_should_charges.payment='0001' and ticket_typ='0001' and tb_should_charges.used='Y' and tb_charges.used='Y' and tb_charges.state=2").empty? && ! TbParty.find_by_sql("select id from tb_parties where recevice_code='#{ca_rc}' and used='Y' and partytype=1 and left(area,3)<>'001'").empty?
          ...
          #c_b = (c7_0001 - c8_0001) * (1 - 0.0672) #2015年之前使用
          c_b = (c7_0001 - c8_0001) * 0.9434
        end
        ...
        if (aribitprog=='0003' || aribitprog=='0004' || aribitprog=='0007'|| aribitprog=='0008') && ! TbShouldCharge.find_by_sql("select tb_should_charges.id from tb_should_charges,tb_charges where tb_should_charges.recevice_code='#{ca_rc}' and tb_should_charges.id=tb_charges.should_id and tb_should_charges.payment='0002' and ticket_typ='0001' and tb_should_charges.used='Y' and tb_charges.used='Y' and tb_charges.state=2").empty? && ! TbParty.find_by_sql("select id from tb_parties where recevice_code='#{ca_rc}'  and used='Y' and partytype=2 and left(area,3)<>'001'").empty?
          ...
          #c_f = (c7_0002 - c8_0002) * (1 - 0.0672) #2015年之前使用
          c_f = (c7_0002 - c8_0002) * 0.9434
        end
  ...
  def list_1
    ...
    unless session[:recevice_code_2] == nil
      ...
      c = "recevice_code='#{session[:recevice_code_2]}' and used='Y' and (p_typ='0001' or p_typ='0002')"
...
#      end
      ...
      # 裁决书报酬
      @r_a = RemunerationAward.find(:all, :conditions => "used='Y' and recevice_code='#{ca_rc}' and p_typ='0001'" )
      for r in @r_a
        if r.typ == "0001"
      ...
      bonus2 = TbBonusPenalty.find_by_sql(
        "select * from tb_bonus_penalties
         where recevice_code='#{ca_rc}' and typ='0002' and used='Y'
        "
      )
      for b2 in bonus2
        @zl4 = User.find_by_code(b2.p).name
        @zlje4 = b2.zc_rmb
      end
  ...
  def list_4
    ...
    unless session[:recevice_code_2] == nil
      ...
      @user = TbBonusPenalty.find_by_sql(
        "select * from tb_bonus_penalties
         where recevice_code='#{session[:recevice_code_2]}' and recevice_code='#{session[:recevice_code_2]}' and typ='0002' and used='Y'
        "
      )
    end
  end
  ...
  def update_5
    ...
    if @remun.used=='Y' and @remun.extend_code=='' and !query_state(session[:recevice_code_2])
      ...
      if @remun.update_attributes(params[:remun])
        ...
        flash[:notice]="修改失败"
        render :action=>"edit_5",:id=>params[:id]
      end
    else
      flash[:notice]="修改失败，本案报酬已经发放！"
      render :text => "<script type='text/javascript'>window.parent.location.reload();</script>"
