...
class UserController < ApplicationController
  ...
  def change_password_do
    ...
    @user.u_t=Time.now()
    if @user.save
      render :text=>"内网密码修改成功！"
    else
      render :action =>"change_password"
    end
  end

  def change_ext_password
    @user=User.find(params[:id])
    @user.password=""

  end

  def change_ext_password_do
    @user=User.find(params[:id])
    @user.ext_password=Digest::MD5.hexdigest(params[:user][:password])
    @user.u=session[:user_code]
    @user.u_t=Time.now()
    if @user.save
      render :text=>"外网密码修改成功！"
    else
      render :action =>"change_ext_password"
    end
  end
