class CaseConsultation3Controller < ApplicationController
  ...
  def new
    ...
    @case.acceptt = params[:apply_acceptt] if params[:apply_acceptt]
  end

  def create
    @case=TbCase.new(params[:case])
    @case.apply_code=params[:apply_code]
...
#        c.finally_limit_dat=c.limit_dat
        ...
        TbDictionary.find(:all, :conditions=>"typ_code='9105' and used='Y' and state='Y'" , :order=>"ind,data_val").each{|dic|
          CaseAttTimeManager.create(:recevice_code=>params[:recevice_code],:typ_code=>dic.data_val)
        }
